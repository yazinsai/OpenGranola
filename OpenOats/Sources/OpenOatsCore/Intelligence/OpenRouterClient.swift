...
actor OpenRouterClient {
    ...
    /// On platforms where SSE streaming is unavailable, falls back to a single non-streaming request.
...
#else
        // Fallback for platforms without URLSession.bytes (e.g. Windows): non-streaming request.
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
