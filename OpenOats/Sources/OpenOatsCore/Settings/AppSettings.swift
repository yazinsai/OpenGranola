...
enum TranscriptionModel: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
        case .whisperBase: "Whisper Base (EN)"
        }
    }

    var downloadPrompt: String {
        "Whisper requires a one-time model download (ggml-base.en.bin)."
    }
    ...
    var localeHelpText: String {
        "Whisper Base (EN) handles English efficiently regardless of locale settings."
    }
}
...
enum KeychainHelper {
    ...
    static func save(key: String, value: String) {
        ...
        #else
        // Fallback for Windows (store in UserDefaults for now until Credential Vault is written)
    ...
    static func delete(key: String) {
        ...
        #else
        UserDefaults.standard.removeObject(forKey: "mock_keychain_\(key)")
        #endif
    }
}
