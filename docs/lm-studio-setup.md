# OpenCassava LM Studio Setup Guide

This guide standardizes the local AI setup for OpenCassava using LM Studio for the chat model and embeddings, plus the built-in Whisper download used by OpenCassava for transcription.
...
Install LM Studio on the same machine where OpenCassava will run.
...
Use this model for suggestions and notes generation inside OpenCassava.
...
Do not download this one in LM Studio. OpenCassava downloads Whisper itself on first run.
...
If your LM Studio configuration uses a different port for embeddings, use that port for the embedding base URL in OpenCassava.

## Configure OpenCassava for LM Studio
...
```text
C:\Users\<your-user>\AppData\Roaming\OpenCassava\settings.json
```
...
- OpenCassava automatically appends `/v1` if it is missing from the base URL.
...
- Add an onboarding screen that explains the three-model setup: LM Studio LLM, LM Studio embeddings, and OpenCassava Whisper.
